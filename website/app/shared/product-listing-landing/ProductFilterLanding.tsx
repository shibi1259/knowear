...
const ProductFilterLanding = (props: Props) => {
  ...
  const handleClearFilters = () => {
    // Reset filters while keeping the selected category
    setPriceFilter({
      low: filters?.price?.low ? Math.floor(filters.price.low) : filters?.price?.low,
      high: filters?.price?.high ? Math.floor(filters.price.high) : filters?.price?.high,
    });
    ...
    setFilterData({
      ...
      category: filterData?.category, // Keep the existing category
      color: null,
      size: null,
    });
...
export default ProductFilterLanding;
