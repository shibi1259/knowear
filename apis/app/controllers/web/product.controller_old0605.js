...
exports.getProductListing = async (req, res) => {
  try {
    // Simplified caching - remove mobile detection overhead
    res.set('Cache-Control', 'public, max-age=600'); // 10 minutes for all devices
    ...
    //Updating search keyword in db if - made asynchronous for better performance
    if (req.body?.search?.length > 0) {
      let body = req.body;
      // Non-blocking search history updates for faster API responses
      setImmediate(async () => {
        try {
          if (userid) {
            ...
            if (searchHistoryDetails) {
              ...
              await searchHistoryService.create({
                searchHistory: [body?.search],
                deviceToken: deviceToken,
              });
            }
          }
        } catch (error) {
          console.error('Search history update failed:', error);
          // Don't fail the API response if search history update fails
        }
      });
    }
